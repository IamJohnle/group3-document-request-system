                            ...
                            <tbody className="divide-y text-sm">
                                {filteredRequests.map((req) => (
                                    <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-muted-foreground">{req.documentType.name}</td>
                                        <td className="px-6 py-4 text-muted-foreground"><span className="truncate max-w-[150px] inline-block">{req.purpose}</span></td>
                                        <td className="px-6 py-4 text-center"><span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{req.copies}</span></td>
                                        <td className="px-6 py-4"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                                        ...
                                        <td className="px-6 py-4 text-right space-x-2">
                                            ...
                                            <Link href={route('admin.requests.show', { docRequest: req.id })} className="text-indigo-600 hover:underline"><Eye size={16} /></Link>
                                            <Link href={route('admin.requests.edit', { docRequest: req.id })} className="text-green-600 hover:underline"><Edit size={16} /></Link>
                                            <button onClick={() => handleDelete(req.id)} className="text-red-600 hover:underline"><Trash2 size={16} /></button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
