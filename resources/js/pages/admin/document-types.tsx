...
const DocumentTypes = ({ documentTypes }: Props) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingItem) {
            put(`/admin/document-types/${editingItem.id}`, {
    ...
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this document type? This action cannot be undone.')) {
            destroy(`/admin/document-types/${id}`);
        }
    };
                    ...
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md animate-in fade-in zoom-in duration-200">
                        <div className="p-6 border-b flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">Fill in the details for this document.</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1.5">Document Name</label>
                                <input
                                    type="text"
                                    value={data.name}
                                    onChange={e => setData('name', e.target.value)}
                                    className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all ${errors.name ? 'border-red-500' : 'border-gray-200'}`}
                                    placeholder="e.g. Official Transcript of Records"
                                    required
                                />
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1.5">Price (PHP)</label>
                                    <input
                                        type="number"
                                        value={data.price}
                                ...
                                <div>
                                    <label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1.5">Requirements</label>
                                    <input
                                        type="text"
                                        value={data.requirements}
                                        onChange={e => setData('requirements', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                                        placeholder="e.g. 2x2 Picture"
                                    />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none min-h-[100px] resize-none"
                            ...
                            <div className="pt-4 flex gap-3">
                                ...
                                <button
                                    type="submit"
                                    disabled={processing}
                                    className="flex-1 px-4 py-2.5 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50"
                                >
                                    {processing ? 'Saving...' : editingItem ? 'Update Type' : 'Create Type'}
                                </button>
                            </div>
                        </form>
                    </div>
